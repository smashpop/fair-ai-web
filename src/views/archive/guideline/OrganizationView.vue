<script setup>
// Components
import OrganizationGuidelineCard  from './components/OrganizationGuidelineCard '

// Utilities
import { ref, watch } from 'vue'
import { fetchOrganizationList } from '@/apis/guideline'
...
async function loadFromServer() {
  ...
  await fetchOrganizationList(serverOptions.value)
    .then((response) => {
    ...
    <v-container class="pa-0" fluid>
      <v-row justify="space-between" align="center" no-gutters>
        <v-col class="d-flex align-center line-height-normal">
          <p class="text-34 font-weight-bold text-black">국제기구</p>
        </v-col>
      ...
      <v-row no-gutters class="justify-space-between">
        ...
        >
          <OrganizationGuidelineCard  :item="item" />
        </v-sheet>
      </v-row>
    </v-container>
  </v-sheet>
